...
static void			ft_cone_col(const t_object obj, t_ray *ray)
{
	...
	if (e.delta < 0.0 && !dot(ray->dir, obj.rot))
		return ;
	e.c = 0 < (-e.b + sqrt(e.delta)) / (2 * e.a) && obj.size.y / 2 < length(ray->pos + ray->dir * e.c - obj.pos) ? (-e.b + sqrt(e.delta)) / (2 * e.a) : ray->t;
	e.delta = 0 < (-e.b - sqrt(e.delta)) / (2 * e.a) && obj.size.y / 2 < length(ray->pos + ray->dir * e.delta - obj.pos) ? (-e.b - sqrt(e.delta)) / (2 * e.a) : ray->t;
	if (e.delta < e.c)
	{
		e.b = e.c;
		e.c = e.delta;
